...
| **Tipo**    | **Número**  | **Resumo**                                                                                                                   |
| ----------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Bug**     | **0000786** | O relatório "Lista de Produtos Vencidos" agora carrega corretamente informações de produtos vencidos, melhorando o controle de estoque. |
| **Bug**     | **0000789** | A distribuição da diferença de valor do kit entre os itens foi corrigida ao usar substituição pela composição, garantindo precisão nos preços. |
| **Bug**     | **0000791** | O sistema agora diferencia corretamente entre IPI e ICMS desonerado, evitando erros na tributação. |
| **Bug**     | **0000792** | O estorno de Ordens de Serviço no segmento de Auto Peças está funcionando sem erros, permitindo correções. |
...
| **Tipo**    | **Número**  | **Resumo**                                                                                                                   |
| ----------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Melhoria** | **0000744** | Melhoria na interface para visualização das diferenças ao realizar a baixa de cartões pendentes, facilitando a reconciliação.   |
| **Melhoria** | **0000758** | Implementação do Demonstrativo de Resultado do Exercício (DRE) para ambos os regimes contábeis (caixa e competência), permitindo uma análise financeira mais abrangente. |
| **Melhoria** | **0000761** | Correções e aprimoramentos no módulo de cotação para melhorar a usabilidade e a precisão das cotações realizadas.              |
| **Melhoria** | **0000762** | Adição da quantidade de estoque na tela de precificação em massa, auxiliando na tomada de decisão de preços com base na disponibilidade. |
| **Melhoria** | **0000763** | Implementação de uma barra de rolagem na tela de relatórios para facilitar a visualização quando os filtros excedem o espaço da tela. |
| **Bug**     | **0000764** | Resolução de discrepâncias entre o Relatório de Vendas por Finalizadora e o DRE, garantindo consistência nos dados financeiros. |
| **Bug**     | **0000765** | Correção de discrepâncias entre o detalhamento e o totalizador do DRE, assegurando a integridade das informações financeiras.  |
| **Bug**     | **0000766** | Correção de erro no relatório de Análise de Vendas Realizadas (Rel 04), permitindo a geração correta dos dados de faturamento.  |
| **Melhoria** | **0000767** | Adição do valor do lucro em reais ao recurso de visualização do custo do pedido ao clicar com o botão direito no módulo de Venda no Balcão, melhorando a análise de rentabilidade. |
| **Bug**     | **0000769** | Correção para evitar que o sistema mantenha o participante no cabeçalho do pedido após a criação de um orçamento, garantindo a correta associação de responsabilidades. |
| **Bug**     | **0000772** | Resolução de problema onde certos pedidos não permitiam a aplicação de descontos nos itens, mesmo com a permissão do departamento, assegurando a flexibilidade de preços. |
| **Melhoria** | **0000774** | Tornar os campos MARCA e MODELO obrigatórios na Ordem de Serviço (OS) do segmento Comercial para melhorar a rastreabilidade e organização dos serviços. |
| **Melhoria** | **0000777** | Implementação de bloqueio na venda de produtos com lote vencido nos módulos EVD/ENP, e ajuste do comportamento na venda EVP para prevenir vendas de produtos impróprios. |
| **Bug**     | **0000768** | Correção de erro ao filtrar relatórios por quantidade de clientes, assegurando que os dados sejam exibidos corretamente.         |
| **Melhoria** | **0000775** | Melhoria na funcionalidade de carregamento de títulos no Relatório de Contas a Receber, especialmente para clientes marcados como SPC, para uma gestão de cobrança mais eficiente. |
| **Bug**     | **0000776** | Correção de erro que ocorria ao tentar efetuar a baixa de títulos, garantindo a funcionalidade correta do sistema financeiro.    |
| **Melhoria** | **0000778** | Adição do nome do vendedor no cabeçalho do relatório da ordem de serviço, melhorando a rastreabilidade e a atribuição de vendas. |
| **Bug**     | **0000779** | Correção de discrepância nos valores apresentados no índice 1 do relatório de Análise de Vendas Realizadas, assegurando a precisão dos dados de faturamento. |

### `11.2024 - Novembro`

| **Tipo**    | **Número**  | **Resumo**                                                                                                                   |
| ----------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Bug**     | **0000771** | Ao fazer a devolução de venda, o usuário é registrado como o responsável pela venda e não como o responsável pela devolução. |
| **Bug**     | **0000748** | O módulo de cotação exibe duas colunas com o código do fabricante; uma delas deve ser substituída pelo código original.       |
| **Bug**     | **0000746** | Garantir que a devolução seja verificada no momento da geração da NF, e não quando o módulo é aberto via atalho Ctrl + D.       |
| **Melhoria**| **0000737** | Ao fazer uma cotação, exibir o código do fabricante conforme cadastrado no item.                                             |
| **Bug**     | **0000756** | Ajustar a quebra de linha do campo "observação" no módulo de pedido de venda.                                                 |
| **Melhoria**| **0000754** | Inserir a coluna "TOTAL COM DESCONTO" na tela do balcão.                                                                    |
| **Bug**     | **0000751** | Ao clicar no ícone de "..." ou apertar a tecla F4, o sistema não abre a tela de produtos.                                    |
| **Melhoria**| **0000749** | Adicionar a extensão .R02 na conciliação de arquivos de retorno.                                                             |
| **Melhoria**| **0000502** | Melhorias na função de ajuste de inventário.                                                                                 |
| **Melhoria**| **0000739** | Ao fechar uma venda pela tela de Pedido de Venda, garantir que a mesma data seja buscada no balcão.                          |
| **Melhoria**| **0000742** | Ajustar a exibição da coluna "Condição de Pagamento" na tela de Indicadores de Venda para que a forma de pagamento seja exibida corretamente. |
| **Melhoria**| **0000743** | Adicionar coluna de valor e subtotal no módulo de perda de mercadoria.                                                       |
| **Melhoria**| **0000745** | Adicionar campos para inserir modelos de pedido de venda.                                                                   |
| **Bug**     | **0000747** | O prejuízo está aparecendo em duplicidade no DRE do período de novembro.                                                     |
| **Melhoria**| **0000750** | Adicionar filtro por participante no relatório "Ranking de Produtos" (Relatório > Produto).                                   |
| **Melhoria**| **0000753** | Incluir filtro de "Motivo de perda" no relatório de Perda.                                                                  |
| **Melhoria**| **0000726** | Incluir campo de unidade de medida no comprovante de entrega parcial da entrega futura.                                      |
| **Bug**     | **0000757** | Implementar tratamento para obter informações da receita e do CMV no plano de contas.                                        |
| **Bug**     | **0000736** | O detalhamento do haver não está considerando os itens devolvidos, mesmo quando há referência da venda na devolução.         |
| **Melhoria**| **0000733** | Incluir setor de produção nas opções de manutenção em massa de produtos.                                                     |
| **Bug**     | **0000735** | O haver não aparece no faturamento manual.                                                                                   |
| **Melhoria**| **0000734** | Incluir uma coluna para o setor de produção no módulo de produtos.                                                           |
| **Melhoria**| **0000740** | Adicionar a extensão .R03 nas leituras de arquivos de retorno.                                                               |
...
| **Bug**     | **0000731** | O sistema não calcula o troco, mesmo com o parâmetro habilitado na estação.                                                 |
| **Melhoria**| **0000729** | Adicionar coluna com o valor da venda atual ao adicionar um novo item na cotação.                                           |
| **Bug**     | **0000728** | Implementar restrição para estorno de pedidos de venda no gerencial com devoluções e títulos já processados.                 |
| **Bug**     | **0000617** | Ao realizar a devolução de uma venda pelo módulo de devolução, o sistema referencia incorretamente o documento de venda.     |
| **Melhoria**| **0000713** | Aumentar a quantidade de casas decimais para o desconto padrão e o desconto máximo no cadastro de produtos.                 |
| **Melhoria**| **0000714** | Criar um acréscimo padrão nas parcelas do cartão de crédito.                                                                 |
| **Bug**     | **0000715** | Bug na importação de cupons no módulo de lançamento.                                                                        |
| **Bug**     | **0000723** | Erro de referência em segunda devolução de venda.                                                                            |
| **Bug**     | **0000724** | Permissão indevida para edição de pedido com status "Fechado".                                                              |
| **Bug**     | **0000725** | Problemas nas formas de pagamento "Outros" e "PIX" em vendas.                                                              |
| **Melhoria**| **0000727** | Incluir indicadores dos totalizadores da nota fiscal no romaneio de entrada analítico - Margens.                             |
